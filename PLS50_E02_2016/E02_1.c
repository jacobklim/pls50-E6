//PLS 50
//Homework E02 - Question 1
//Author: Iakovos Klimenof
//Description: Bus reservation extended for E01
...
void readBusFile(FILE *file, char *bus_plate, int *seats, Passenger occupancy[]);
void printBusInfo(char *bus_plate, int seats);
void initializePassengerArray(Passenger occupancy[]);
void checkEmptySeats(Passenger occupancy[], int seats);
int checkIfSeatIsEmpty(Passenger occupancy[], int seat_number);
void reserveSeatByNumber(Passenger occupancy[], int seats);
int checkNameOrTelephone(Passenger occupancy[], int seats);
void printBusStatus(Passenger occupancy[], int seats);
void cancelReservationByNumber(Passenger occupancy[], int seats);
void refreshBusFile(FILE **file, Passenger occupancy[],char *bus_plate, int seats);
...
int main(void){
    ...
    //Initialization of the array of structures
    Passenger occupancy[MAX_SEATS];
    //Make all the seats empty
    initializePassengerArray(occupancy);

    //Open the file bus,txt, if no file with name bus.txt
    //return OPEN_FILE_ERROR
    if(openFile(&fp) == OPEN_FILE_SUCCESS){
        printf("File OK\n");
        //Read the bus file and make the appropriate changes to the variables
        readBusFile(fp, bus_plate, &seats, occupancy);
    }
    else{
    ...
    while(user_selection = getUserSelection()){
        ...
        case 1:
            printf("%d\n",user_selection);
            checkEmptySeats(occupancy, seats);
            break;
        case 2:
            printf("%d\n",user_selection);
            reserveSeatByNumber(occupancy, seats);
            break;
        case 3:
            printf("%d\n",user_selection);
            if(checkNameOrTelephone(occupancy, seats)==NAME_OR_NUMBER_OK){
                printf("There is a reservation with this Name or Phone number!\n");
            }
            break;
        case 4:
            printf("%d\n",user_selection);
            cancelReservationByNumber(occupancy, seats);
            break;
        case 5:
            printf("%d\n",user_selection);
            printBusStatus(occupancy, seats);
            break;
    ...
    //Refresh the bus.txt before exit
    refreshBusFile(&fp, occupancy, bus_plate, seats);
    return 0;
}
...
/** Function:       readBusFile()
    Description:    Reads the file with fscanf()
                    and updates the bus_plate, seats and
                    the array of Passenger structs occupancy.
*/
void readBusFile(FILE *file, char *bus_plate, int *seats, Passenger  occupancy[]){
    //Helper variables
...
void reserveSeatByNumber(Passenger occupancy[], int seats){
        ...
        }//in case od invalid user selection
        else{
